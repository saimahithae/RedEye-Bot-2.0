...
class RedEyeAutoUITests: XCTestCase {
    ...
    func testBookRideFromSnellToHome() throws {
        ...
        //let destinationAddress = app.staticTexts["2 Presby Pl, Roxbury"]
        
        if confirmButton.waitForExistence(timeout: 5) {
            XCTAssertTrue(confirmButton.exists, "Confirm button not found")
            confirmButton.tap()
            // After tapping 'Book This Ride', check if the 'OK' button exists
            
        } else if okButton.waitForExistence(timeout: 5) {
            XCTAssertTrue(okButton.exists, "'OK' button was found, ride not confirmed")
                okButton.tap() // Handling the case if OK needs to be tapped for some reason
        }
        else {
            XCTFail("The 'Book This Ride' button was not found")
        }
    }
}
